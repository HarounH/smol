...
class ModelInternalsLogger:
    ...
    def _record_tensor(self, name: str, tensor: torch.Tensor | None) -> None:
        ...
        if tensor.requires_grad:
            tensor.register_hook(
                lambda grad, metric_name=name: self._record_grad(metric_name, grad)
            )
        ...
        for layer_index, layer in enumerate(encoder_layers):
            ...
            def attn_pre_hook(module, inputs, prefix=layer_prefix):
                ...
                if (
                    ...
                    self._record_tensor(
                        f"{prefix}/q_proj", F.linear(q_input, q_weight, q_bias)
                    )
                    self._record_tensor(
                        f"{prefix}/k_proj", F.linear(q_input, k_weight, k_bias)
                    )
                    self._record_tensor(
                        f"{prefix}/v_proj", F.linear(q_input, v_weight, v_bias)
                    )
                    return
                if all(
            ...
            self._handles.append(
                layer.self_attn.register_forward_pre_hook(attn_pre_hook)
            )
            self._handles.append(
    ...
    def finalize_step(self) -> dict[str, float]:
        for layer_index, layer in enumerate(getattr(self.model.encoder, "layers", [])):
            layer_prefix = f"internals/layer_{layer_index}"
            if (
                hasattr(layer.self_attn, "in_proj_weight")
                and layer.self_attn.in_proj_weight is not None
            ):
                q_weight, k_weight, v_weight = layer.self_attn.in_proj_weight.chunk(
                    3, dim=0
                )
            ...
            self._metrics.update(
                tensor_summary_metrics(f"{layer_prefix}/q_weight", q_weight)
            )
            self._metrics.update(
                tensor_summary_metrics(f"{layer_prefix}/k_weight", k_weight)
            )
            self._metrics.update(
                tensor_summary_metrics(f"{layer_prefix}/v_weight", v_weight)
            )
            if q_grad is not None:
                self._metrics.update(
                    tensor_summary_metrics(f"{layer_prefix}/q_weight/grad", q_grad)
                )
                self._metrics.update(
                    tensor_summary_metrics(f"{layer_prefix}/k_weight/grad", k_grad)
                )
                self._metrics.update(
                    tensor_summary_metrics(f"{layer_prefix}/v_weight/grad", v_grad)
                )
