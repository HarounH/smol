...
class CustomSDPA(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v):
        ...
        scale = 1.0 / torch.sqrt(
            torch.tensor(q.size(-1), dtype=q.dtype, device=q.device)
        )
...
def run_attention(
    ...
    *,
    ...
    Q = torch.randn(
        batch_size, num_heads, q_seq_len, head_dim, device="cuda", dtype=dtype
    )
    K = torch.randn(
        batch_size, num_heads, k_seq_len, head_dim, device="cuda", dtype=dtype
    )
    V = torch.randn(
        batch_size, num_heads, k_seq_len, head_dim, device="cuda", dtype=dtype
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run attention on random Q, K, V tensors."
    )
    ...
    parser.add_argument(
        "--q_seq_len", type=int, required=True, help="Q sequence length"
    )
    parser.add_argument(
        "--k_seq_len", type=int, required=True, help="K sequence length"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="sdpa",
        help="Attention kernel to use (default: sdpa)",
    )
    parser.add_argument(
        "--dtype",
        type=str,
    ...
    parser.add_argument(
        "--seed", type=int, default=1337, help="Random seed for torch and CUDA"
    )
    parser.add_argument(
        "-c",
        "--check_correctness",
        action="store_true",
        help="Compare kernel output against PyTorch SDPA",
    )
    parser.add_argument(
        "--warmup",
        type=int,
        default=0,
        help="Number of warmup iterations to run before executing the measured pass",
    )
